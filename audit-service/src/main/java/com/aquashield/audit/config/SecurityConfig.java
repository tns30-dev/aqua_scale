package com.aquashield.audit.config;
...
/**
 * Resource-service security (same platform pattern as project-service): local JWT
 * verification + fail-closed Redis snapshot. The ENTIRE query API is platform-admin
 * only (spec: "Expose admin audit queries") — enforced via @PreAuthorize per endpoint.
 */
@Configuration
@EnableWebSecurity
  ...
  JwtVerifier jwtVerifier(@Value("${aquashield.jwt.public-key-pem}") String publicKeyPem,
    ...
    if (publicKeyPem == null || publicKeyPem.isBlank()) {
      throw new IllegalStateException(
          "JWT_PUBLIC_KEY_PEM is required for audit-service (Identity's public key)");
    }
