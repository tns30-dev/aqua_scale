...
- **Current focus:** THREE services live (identity + project + sensor), 63 tests green. Ingestion is now UNBLOCKED (ResolveDevicePort + GetDeviceValidationMetadata + PayloadHmac all ready). Next candidates: ingestion-service (the event-driven heart), pond-service, or CI skeleton — user directs.
