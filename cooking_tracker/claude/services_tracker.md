...
- **Current focus:** FOUR services live (identity + project + sensor + ingestion), 72 tests green. The IoT chain works locally end-to-end: HMAC-signed payload → Pub/Sub → pipeline → reading.ingested. Next candidates: notification-service (consumes reading.ingested, thresholds from Project gRPC), pond-service, or CI skeleton — user directs.
