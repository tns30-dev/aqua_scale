...
- **Current focus:** The data layer is in heavy production use locally. Next data-side work: event schema files (`shared-api/events/*.v1.json`), Bigtable raw-store impl behind the ReadingStore seam, then Terraform for the cloud instances.
...
|---|---|---|---|---|
| Cloud SQL PostgreSQL primary | 🟨 | Local equivalent in production use: 6 service schemas with Flyway migrations + service DB roles; schema-per-service ownership enforced (no cross-schema access in code — analytics reads ONLY through gRPC seams, owns no schema). Chart config tables live in `project` schema (V3). Cloud instance pending Terraform | `docs/evidence/local-foundation/` + per-service ITs | 2026-06-04 |
