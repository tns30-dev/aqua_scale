...
| Argo CD rollout | DONE | Argo CD synced the private GitHub repo at revision `85b6bab58622ed9357018deac4dd6ca5f8f8d262`, targets `k8s/overlays/dev-managed-public`, reports `Synced/Healthy`, and rolled `api-edge-proxy` config `2026-06-05-cors-frontend-origin`. | `../main/cd.md`, `../main/gke.md`, `../../k8s/argocd/aquashield-dev-application.yaml`, `../../docs/evidence/gitops/2026-06-05-argocd-dev-managed-rollout.md`, `../../docs/evidence/public-edge/2026-06-05-public-edge-live-rollout.md`, `../../docs/evidence/public-edge/2026-06-05-api-edge-cors-fix.md` | 2026-06-05 |
...
| k6 load, stress, growth, and WebSocket tests | IN_PROGRESS | k6 tooling exists and local 50-user evidence is recorded. Cloud-native evidence is next and should run only from Kubernetes Job or GitHub Actions manual/performance-test execution against the deployed gateway. | `../../.github/workflows/perf.yml`, `../../loadtests/`, `../../docs/evidence/performance/`, `../../docs/second_evidence/performance/`, `../main/ci.md` | 2026-08-08 |
| Demo evidence | IN_PROGRESS | Managed runtime smoke, AWS IoT/Lambda live smoke, public HTTPS API edge live smoke, Firebase Hosting custom-domain live deploy, frontend-to-API CORS proof, CI/SAST recovery, and HTTPS static-IP reservation evidence are recorded. DAST and performance evidence remain. | `../../docs/evidence/` | 2026-06-05 |
...
| Firebase custom-domain activation | PASS; `www.aquashield.live` CNAME and ACME TXT are visible locally and from `8.8.8.8`; HTTPS GETs for `/` and `/login` return the AquaShield SPA. | 2026-06-05 |
| API edge CORS rollout | PASS; CI run `27007178776` and deploy-handoff run `27007224758` succeeded, Argo synced revision `85b6bab`, preflight from `https://www.aquashield.live` returns `204` with allow-origin, login returns `200` with allow-origin, and unknown origin receives no allow-origin header. | 2026-06-05 |
...
| 2026-06-05 | Deployed the frontend to Firebase Hosting with live API/WSS endpoints, verified the default Hosting URL and SPA rewrite, and activated `https://www.aquashield.live` for the public frontend. |
| 2026-06-05 | Shipped the API edge CORS fix through GitHub Actions and Argo CD revision `85b6bab`; browser-origin preflight/login checks now pass for `https://www.aquashield.live`. |
