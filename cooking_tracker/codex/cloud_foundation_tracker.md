...
| Public API edge manifests | DONE | `dev-managed-public` is live through Argo on revision `85b6bab`: GKE Gateway named address, HTTP redirect, pre-shared managed certificate, `/api` and `/ws` routes to `api-edge-proxy`, proxy health check, business-flow smoke, and frontend-origin CORS proof passed. | `../../k8s/base/services/api-edge-proxy/`, `../../k8s/overlays/dev-managed-public/`, `../../docs/evidence/public-edge/2026-06-05-public-edge-live-rollout.md`, `../../docs/evidence/public-edge/2026-06-05-api-edge-cors-fix.md` | 2026-06-05 |
...
| API edge CORS rollout | PASS; Argo reports `Synced/Healthy` at revision `85b6bab`, `api-edge-proxy` is ready with config `2026-06-05-cors-frontend-origin`, and live CORS probes pass for the Firebase custom/default origins. | 2026-06-05 |
...
| 2026-06-05 | Rolled the API edge CORS allow-list through Argo CD so the Firebase custom/default origins can call `https://api.aquashield.live` from browser JavaScript. |
