...
| Public CORS allow-list | PASS; `api-edge-proxy` allows `https://www.aquashield.live` and `https://aerobic-guide-498413-u6.web.app`, keeps `/api/auth/me` fail-closed at 401 without a token, and returns no allow-origin header for `https://evil.example`. | 2026-06-05 |
...
| 2026-06-05 | Fixed frontend-to-API CORS at the edge with an explicit Firebase-origin allow-list and recorded positive plus negative origin checks. |
