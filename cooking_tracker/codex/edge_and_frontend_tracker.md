...
- Current state: Local gateway route rehearsal and frontend Java-service wiring are in progress. Dev Kustomize now points all nine backend services to Artifact Registry images; cloud edge and Firebase Hosting are not deployed.
- Current test: Frontend tests/build plus Kustomize render for Gateway/HTTPRoute manifests and all-service image substitution.
- Next test: Apply GKE/network/Cloud Armor first, then deploy Gateway/LB/WSS edge and Firebase preview/staging.
...
| GCP API edge | IN_PROGRESS | Gateway API and HTTPRoute skeleton route `/api/**` and `/ws` to implemented services; dev overlay now has Artifact Registry images. Cloud LB is not provisioned yet. | `../../k8s/base/edge/`, `../../k8s/overlays/dev/kustomization.yaml`, `../../docs/evidence/k8s-audit-service/2026-06-04-audit-kustomize-validation.md` | 2026-06-05 |
